...
class ContadorConsultas {
	...
	def seRealizoBusqueda(List<Receta> recetas) {
		recetas.forEach [ receta |
			if (!recetasMasConsultadas.containsKey(receta)) {
				recetasMasConsultadas.put(receta, 1)
				recetasAux.add(receta)
			} else {
				recetasMasConsultadas.put(receta, recetasMasConsultadas.get(receta) + 1)
			}
		]
	}
	...
	def ordenar() {
		...
		recetasAux.forEach[ rec | contadores.add(new ContadorRecetas(rec, recetasMasConsultadas.get(rec))) ]
